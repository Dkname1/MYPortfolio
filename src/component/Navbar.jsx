    ...
    <>
      <div className="flex justify-between py-6 px-14 shadow-md  w-[600px] sm:w-[100%]">
        <div className=" text-2xl md:text-3xl font-semibold">
          ...
          <ReactTyped
            className="block md:hidden text-fuchsia-900"
            strings={[`${logo.title}`]}
            typeSpeed={100}
            backSpeed={80}
            loop
        ...
        <div
          className={`md:static  absolute bg-gray-100  md:bg-white 
duration-500 ${
            toggle ? `top-[-100%]` : `top-[9.9%]`
          }  left-0  w-[600px] sm:w-full md:w-auto  flex md:items-center h-auto md:min-h-fit `}
        >
            ...
            {navmenu.map((list) => {
              return (
                <li key={list.id} className="mt-5 md:mt-0 hover:text-gray-600">
                  <Link onClick={() => setLine(`${list.argu}`)} to={list.Link}>
                    {list.title}
                  </Link>
                  {line === `${list.argu}` ? (
                    <hr className="w-[60%] h-[3px]  bg-fuchsia-900 top-8" />
                  ) : (
                    <></>
                  )}
                </li>
...
<li className='mt-5 md:mt-0 '>
     ...
     {line === 'skills' ? <hr className='flex w-[80%] h-[3px] bg-[#FF4141] top-8'/> :<></>}
        ...
        <div className="flex items-center">
          <button className=" md:text-xl  md:font-semibold border  shadow-sm border-gray-300  px-4 md:px-6 py-2 rounded-full text-fuchsia-900 hover:text-fuchsia-600 font-semibold hover:bg-gray-50">
            {button.title}
          </button>
          <div className="ml-6 md:m-0">
            {toggle ? (
              <RiMenuAddFill
                ...
                className="text-3xl  text-black md:hidden block"
              />
            ) : (
              <RxCross2
                onClick={() => setToggle(!toggle)}
                className="text-3xl md:hidden block text-black"
              />
            )}
          </div>
        </div>
